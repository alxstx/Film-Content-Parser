            ...
            if link != '':
                ...
                if items != [] and testnumber != None and True not in notneccesarycheck:
                    ...
                    for item in items:
                        ...
                        print(number)
                        try:
                            driver1.find_element_by_xpath('//*[@id="rsvg__ive--info"]/path').click()
                        ...
                        if filmname != '' and number != '':
                            ...
                            pass
                    if imdboffon.lower() == 'on':
                        for rel in title_lists:
                            driver1.get('https://www.imdb.com/?ref_=nv_home')
                            driver1.find_element_by_xpath(
                                '/html/body/div[2]/nav/div[2]/div[1]/form/div[2]/div/input').send_keys(rel)
                            time.sleep(12)
                            html55 = driver1.page_source

                            soup55 = BeautifulSoup(html55, 'lxml')
                            try:
                                released2 = soup55.find('div',
                                                        'searchResults__ResultTextItem-sc-1pmqwbq-2 lolMki _1DoAqrviL4URifsx8tYz_V').text
                            except:
                                released2 = ''
                            print(released2)
                            released_lists.append(released2)
                        for rell in released_lists:
                            all_data['Дата премьеры'].append(rell)
                            if 'movies' or 'films' or 'dramas' in title.lower():
                                data['Дата премьеры'].append(rell)
                            elif 'shows' or 'series' or 'cartoons' or 'sitcoms' in title.lower():
                                data4['Дата премьеры'].append(rell)
                        ddd = func()
                        if ddd == 'stop':
                            end_that()
...
if __name__ == '__main__':
    try:
        for i in range(n):
            t = threading.Thread(name='Test {}'.format(i), target=get_all_data)
            t.daemon = True
            t.start()
            time.sleep(2)
